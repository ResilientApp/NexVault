...
import { injectKeyValuePair } from "../../utils/utils";
...
export const determineInitialRouteForState = async (state: RootState) => {
  ...
  const selectedNetworkID = network.selectedNetworkIdx;
  const selectedNetwork = selectedNetworkID
    ? allNetworks[selectedNetworkID]
    : allNetworks[0];
  let selectedAccountAddress = selectedNetwork.selectedAccount;
  if (!selectedAccountAddress) {
    await router.replace(`/network/${selectedNetworkID}/account/add`);
    return;
  }
  ...
  await injectKeyValuePair("user_id", selectedAccountAddress);
  await router.replace(
    `/network/${selectedNetworkID}/account/${selectedAccountAddress}`
  );
};
...
//   }
//   // Resolve selectedNetwork, fallback to first network.
//   const selectedNetworkIdx = network.selectedNetworkIdx || 0;
//   const selectedNetwork = allNetworks[selectedNetworkIdx]
...
//   // Successfully resolved a network and an account. Goto account home.
//   injectKeyValuePair("user_id", selectedAccountAddress);
//   console.log(selectedAccountAddress)
//   await router.replace(
//     `/network/${selectedNetwork.chainId}/account/${selectedAccountAddress}`
//   );
// };

// export default router;

// introduce the ROK token
// two chains, concurrent mining
