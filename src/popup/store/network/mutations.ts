...
// import { handleAccountCreated } from "../../../utils/event_utils";
...
export type Mutations<S = State> = {
  [MutationTypes.ADD_NETWORK](state: S, network: Network): void;
  [MutationTypes.SET_INITIAL_STATE](state: S, newInitialState: S): void;
  [MutationTypes.SET_SELECTED_NETWORK](state: S, selectedNetworkIdx: number): void;
  [MutationTypes.SET_SELECTED_ADDRESS](
    state: S,
    payload: { networkId: number; address: string }
  ): void;
  [MutationTypes.ADD_ACCOUNT](state: S, wallet: TokenWallet): void;
  // [MutationTypes.MODIFY_NETWORK](
  //   state: S,
  //   payload: { networkId: string; partialNetwork: Partial<Network> }
  // ): void;
...
export const mutations: MutationTree<State> & Mutations = {
  ...
  [MutationTypes.SET_SELECTED_NETWORK](state, selectedNetworkIdx) {
    state.selectedNetworkIdx = selectedNetworkIdx;
  },
  [MutationTypes.SET_SELECTED_ADDRESS](state, payload) {
    state.networks[state.selectedNetworkIdx].selectedAccount = payload.address
  },
  [MutationTypes.ADD_ACCOUNT](state, wallet) {
    state.networks[state.selectedNetworkIdx].accounts[wallet.getAddress()] = wallet
  }
  // [MutationTypes.MODIFY_NETWORK](state, payload) {
  //   Object.assign(state.networks[payload.networkId], payload.partialNetwork);
  // },
